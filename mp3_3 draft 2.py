...
class TeeClass:
    ...
    def for_display(self, tokens):
        ...
        for j in range(len(sep_str)):
            ...
            rgx = r'[^;]+'
            rgx_try = r'(\+=|\*=|/=|-=|;|\*|\+|\-|=)+'
            for outer_match in re.finditer(rgx, temp_str):
                temp_try = outer_match.group()
                forStr += temp_try
                # print(temp_try)
                if re.search(r'[\*\=\+\-]', temp_try):
                  for inner_match in re.finditer(rgx_try, temp_try):
                      if inner_match.group().strip() != '':  # Changed to check for non-empty strings
                        #   print(f"wow: {inner_match.group()}")
                          ctr_instruc += 1

                # print(f"try: {ctr_instruc}")
                # rgx_last = r'[^=+-;]+'
                # mulStr= ' '
