...
public:
	...
	void ifDisplay(string);
	void forDisplay(vector<string>);
	void display(vector<string>);
	int count = 0;
};


string teeClass::removeSpace(string str) {
...
void teeClass::ifDisplay(string temp) {
	this->count++;
...
void teeClass::forDisplay(vector<string>tokens) {
	int ctrInstruc = 0;
// 	cout<<"for:"<<endl;
...
// 				cout<<"for statements continued:"<<endl;
			...
			regex rgxLast("[^=+-;]+");
			auto beginLast = sregex_iterator(forStr.begin(), forStr.end(), rgxLast);
			auto endLast = sregex_iterator();
			for (sregex_iterator i = beginLast; i != endLast; ++i) {
				smatch match = *i;
			
				// cout<<"asdas: "<<removeSpace(match.str())<<endl;
...
void teeClass::display(vector<string>tokens) {
	// teeClass statementStr;
...
// 			cout<<temp<<endl;
	...
	if(!forFlag)
	    cout<<"T(n): "<<this->count<<endl;
}

void teeClass::tokenize(const string snippet) {
	...
	if (snippet.find("for") != string::npos) {
		...
		for (sregex_iterator i = begin; i != end; ++i) {
			smatch match = *i;
			// cout<<"sample: "<<match.str()<<endl;
			if (match.str().find("=") != string::npos || match.str().find("<<") != string::npos || match.str().find(">>") != string::npos || match.str().find("<") != string::npos || match.str().find(">") != string::npos) {
				if (match.str().find(",") == string::npos) {
					tempStr += ",";  // Append comma before concatenating
				}
		...
		// cout<<tempStr<<endl;
		regex rgxLast("[^+-;]+");
		auto beginLast = sregex_iterator(tempStr.begin(), tempStr.end(), rgxLast);
		auto endLast = sregex_iterator();
		for (sregex_iterator i = beginLast; i != endLast; ++i) {
			smatch match = *i;
			tokens.push_back(match.str());
...
// 		cout<<"stack: "<<tokens[i]<<endl;
// 	}
	// cout<<"T(nsda): "<<tokenizeString.count<<endl;
}
